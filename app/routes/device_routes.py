...
from app.dependencies.auth_dependencies import get_current_active_user, require_staff

router = APIRouter(prefix="/devices", tags=["Devices"], dependencies=[Depends(get_current_active_user)], responses={401: {"description": "JWT ausente o inválido"}, 403: {"description": "Permisos insuficientes"}, 404: {"description": "Dispositivo inexistente"}, 400: {"description": "Serie duplicada o actualización vacía"}, 409: {"description": "Dispositivo con préstamos"}})
...
def create_device(payload: DeviceCreate, db: Session = Depends(get_db), staff=Depends(require_staff)):
...
def replace_device(device_id: int, payload: DeviceUpdate, db: Session = Depends(get_db), staff=Depends(require_staff)):
...
def patch_device(device_id: int, payload: DevicePatch, db: Session = Depends(get_db), staff=Depends(require_staff)):
...
def delete_device(device_id: int, db: Session = Depends(get_db), staff=Depends(require_staff)):
...
def device_loans(device_id: int, db: Session = Depends(get_db), staff=Depends(require_staff)):
